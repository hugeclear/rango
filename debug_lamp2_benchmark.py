...
class LaMP2Evaluator:
    ...
    def __init__(
        ...
        self.max_samples = max_samples  # 評価するサンプル数を制限
        ...
        print(
            f"   テストサンプル数: {len(self.test_data)} (評価対象: {min(len(self.test_data), self.max_samples)}件)"
        )
    ...
    def _load_test_data(self) -> List[Dict]:
        ...
        possible_paths = [
            ...
            Path(
                "chameleon_prime_personalization/data/processed/LaMP-2/merged.json"
            ),  # フルパス
            Path(
                ...
                try:
                    ...
                    if isinstance(sample, dict):
                        ...
                        if "id" in sample and "output" in sample:
                            # 標準形式
                            ground_truth = {
                                str(ans["id"]): str(ans["output"]).lower().strip()
                                for ans in answers
                            ...
                            # 代替形式
                            ground_truth = {
                                str(ans["id"]): str(ans["answer"]).lower().strip()
                                for ans in answers
                                if isinstance(ans, dict)
                                and "id" in ans
                                and "answer" in ans
                        ...
                        ground_truth = {
                            str(i): ans.lower().strip() for i, ans in enumerate(answers)
                        }
                        print(
                            f"   📝 文字列形式として処理（インデックスをIDとして使用）"
                        )
                    else:
    ...
    def evaluate_baseline_llm(self, model: str = "gpt-3.5-turbo") -> EvaluationResult:
        ...
        # サンプル数を制限
        test_samples = self.test_data[: self.max_samples]
        print(f"   評価対象: {len(test_samples)} サンプル")
        ...
        for i, sample in enumerate(test_samples):
            print(
                f"   進捗: {i+1}/{len(test_samples)} - ID: {sample.get('id', 'unknown')}"
            )
...
Tag:"""
        ...
        return self._calculate_metrics(
            "Baseline_LLM", predictions, inference_time, test_samples
        )
    ...
    def evaluate_chameleon(
        ...
        for i, sample in enumerate(test_samples):
            ...
            edited_embedding = (
                original_embedding + alpha * self.theta_p - beta * self.theta_n
            )
        ...
        inference_time = time.time() - start_time
        return self._calculate_metrics(
            "Chameleon", predictions, inference_time, test_samples
        )
    ...
    def _calculate_metrics(
        ...
        for i, sample in enumerate(test_samples):
            ...
            if sample_id in self.ground_truth:
                ...
                print(f"   ⚠️  ID {sample_id} の正解データが見つかりません")

        if not true_labels:
            print(f"   ⚠️  有効な正解データがありません。予測のみ表示します")
            # 正解データがない場合でも結果を返す
        ...
        for i, (sample, pred, true) in enumerate(
            zip(
                test_samples[:5],
                predictions[:5],
                true_labels[:5] if true_labels else [],
            )
        ):
            print(
                f"     {i+1}. ID:{sample['id']} 予測:{pred} 正解:{true if true_labels else 'N/A'}"
            )
    ...
    def _generate_report(self, results: Dict[str, EvaluationResult]):
        ...
        for method, result in results.items():
            if result:
                ...
                print(
                    f"   正解数:      {result.correct_predictions}/{result.total_samples}"
                )
    ...
    def _plot_comparison(self, results: Dict[str, EvaluationResult]):
        ...
        print(
            f"📈 性能比較グラフ保存: {self.output_dir / 'performance_comparison.png'}"
        )
...
def main():
    ...
    parser.add_argument(
        "--data_path",
        default="./chameleon_prime_personalization/data",
        help="データディレクトリのパス",
    )
    parser.add_argument(
        "--output_dir", default="./evaluation_results", help="結果出力ディレクトリ"
    )
    parser.add_argument(
        "--max_samples",
        type=int,
        default=10,
        help="評価するサンプル数の上限（デフォルト: 10）",
    )
    args = parser.parse_args()

    # 評価実行
    evaluator = LaMP2Evaluator(args.data_path, args.output_dir, args.max_samples)
    results = evaluator.run_full_evaluation()

    print("\n✅ 自動ベンチマーク評価完了!")
    print(f"📁 結果は {args.output_dir} に保存されました")
    print(f"📊 評価サンプル数: {args.max_samples}")
